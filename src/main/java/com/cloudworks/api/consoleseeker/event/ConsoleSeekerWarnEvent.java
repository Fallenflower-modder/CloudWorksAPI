 ...
 * WARN 级别日志事件。
 * <p>
 * 当 ConsoleSeeker 捕获到 WARN 级别的日志且通过内部过滤单元时，此事件会被发布到
 * NeoForge {@code EVENT_BUS}。下游模组可通过 {@code @SubscribeEvent} 订阅。
 * </p>
 */
public class ConsoleSeekerWarnEvent extends ConsoleSeekerLogEvent {

    public ConsoleSeekerWarnEvent(String loggerName, String message,
                                  long timestamp, String threadName, String thrownString,
                                  Set<DirectedDeliveryTag> tagSet) {
        super(loggerName, Level.WARN, message, timestamp, threadName, thrownString, tagSet);
    }
}